...
public class Page {
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			...
			sb.append("	<li><select title='显示条数' id='pageShowCountSelect' style=\"width:55px;float:left;\" onchange=\"changeCount(this.value)\">\n");
			//sb.append("	<option value='"+showCount+"'>"+showCount+"</option>\n");
			int[] showCountArr = {10,20,30,40,50,60,70,80,90,99};
			for(int i = 0; i < showCountArr.length; i++) {
				if(showCount == showCountArr[i]) {
					sb.append("	<option value='"+showCountArr[i]+"' selected>"+showCountArr[i]+"</option>\n");
				} else {
					sb.append("	<option value='"+showCountArr[i]+"'>"+showCountArr[i]+"</option>\n");
				}
			}
			sb.append("	</select>\n");
			sb.append("	</li>\n");
			...
			//页码输入框回车事件绑定
			sb.append("$(function() {\n");
			sb.append("$(\"#toGoPage\").unbind('keydown').bind('keydown',function(event){\n");
