	...
	<head>
		...
		<script type="text/javascript">
			...
			function save(){
				...
				if($("#excel").val()=="" || document.getElementById("excel").files[0] =='请选择'+uploadFileType+'格式的文件'){
					$("#excel").tips({
						side:3,
			            msg:'请选择文件',
			            bg:'#AE81FF',
			...
			function fileType(obj){
				...
				var uploadFileType = $("#uploadFileType").val();
			    if(fileType != "."+uploadFileType){
			    	$("#excel").tips({
						side:3,
			            msg:'请上传'+uploadFileType+'格式的文件',
			            bg:'#AE81FF',
			            time:3
			        });
			    	$("#excel").val('');
			    	document.getElementById("excel").files[0] = '请选择'+uploadFileType+'格式的文件';
			    }
			}
		</script>
	</head>
<body>
	<form action="${pd.url }" name="Form" id="Form" method="post" enctype="multipart/form-data">
		<input type="hidden" name="uploadFileType" id="uploadFileType" value="${pd.uploadFileType }"/>
		<div id="zhongxin">
		...
		$(top.hangge());
		$(function() {
			var uploadFileTypeOnLoad = $("#uploadFileType").val();
			//上传
			$('#excel').ace_file_input({
				no_file:'请选择'+uploadFileTypeOnLoad+' ...',
				btn_choose:'选择',
				btn_change:'更改',
				droppable:false,
				onchange:null,
